...
export const AppNext: React.FC = () => {
  ...
  const extract = useCallback(() => {
    ...
    const { members, grid } = extractStructuralModel(st.dxfEntities, st.dxfLayers, st.dxfTransform, { thicknessProfile: profile });
  ...
  return (
    <div className="w-screen h-screen flex flex-col bg-zinc-950 overflow-hidden text-zinc-100">
      ...
      <div className="flex-1 w-full relative flex overflow-hidden">
        ...
        <RightDock tab={tab} setTab={setTab} onExtract={extract} profile={profile} setProfile={setProfile} />
      </div>
      <BottomBar />
      <Analytics />
    </div>
