...
> 기준 버전: **v0.12.0** · 검증일: **2026-06-23** · 배포: Vercel (`stru-xure-ai.vercel.app`)
...
- **CAD 구조 부재 추출(보기→구조화)**: 보이는 벽/기둥 레이어의 DXF 지오메트리를 **편집 가능한 구조 부재(WALL=빨강/COLUMN=파랑)** 로 변환. 렌더 도면과 픽셀 단위로 정합, 추출 후 선택·이동·정점편집·삭제 가능. (레이어 키워드: `WALL/COL/벽/기둥/柱/墙/牆/RC/옹벽`)
  - **조적벽(masonry) 자동 제외**: `MASONRY/조적/벽돌/BRICK/磚/砌` 레이어는 비내력으로 보고 구조 부재에서 제외
  - **기둥 = 중심점+단면(사각형)**: 기둥 외곽을 바운딩박스 사각형으로 객체화 (네이티브 선택·크기조절)
  - **기둥 중복 정리 + 그리드 스냅**: 같은 위치 중복 기둥 병합, 축선(AXN/축/軸) 레이어에서 그리드를 추출해 기둥 중심을 가까운 교점(±20px)에 정렬
- **중심선 자동 생성(CAD 대응)**: 마주보는 벽 쌍에서 중심선 추출. **수직거리+중첩 기반 페어링**, CAD 추출 벽은 `dxfTransform.scale`로 두께 임계값(약 60~600mm)을 px 환산. **동일선상 인접 중심선은 병합**해 연속 축선으로 연결
