...
- **객체 편집**: SELECT 모드에서 도형 선택·이동(기하적 히트 판정), **선(LINE)은 양 끝점을 끌어 정점(Vertex) 편집** + 전체 이동, 사각형/원/삼각형 평행이동 — 변경 좌표는 스토어에 영속화
...
- **도형 크기·회전 편집**: 선은 정점 편집, 모든 도형은 이동 가능하나 사각형/원/삼각형의 크기조절·회전은 미구현. AI 폴리곤은 비편집(가이드 표시 전용)
