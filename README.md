...
> 기준 버전: **v0.11.0** · 검증일: **2026-06-23** · 배포: Vercel (`stru-xure-ai.vercel.app`)
...
- **CAD 구조 부재 추출(보기→구조화)**: 보이는 벽/기둥 레이어의 DXF 지오메트리를 **편집 가능한 구조 부재(WALL=빨강/COLUMN=파랑)** 로 변환. 렌더 도면과 픽셀 단위로 정합, 추출 후 선택·이동·정점편집·삭제 가능. (레이어 키워드: `WALL/COL/벽/기둥/柱/墙/牆/RC/옹벽`)
  ...
  - **기둥 = 중심점+단면(사각형)**: 기둥 외곽을 바운딩박스 사각형 1개로 객체화 (네이티브 선택·크기조절)
- **중심선 자동 생성(CAD 대응)**: 마주보는 벽 쌍에서 중심선 추출. **수직거리+중첩 기반 페어링**, CAD 추출 벽은 `dxfTransform.scale`로 두께 임계값(약 60~600mm)을 px 환산해 정확히 페어링
