...
- **E2 절점 병합 tolerance화(v0.42.0)** [`buildStructuralModel.nodeId`] — 자유단 58개의 최대 원인.
  ...
  - 📌 **남은 자유단 40 = Ⓑ ~17 + Ⓒ 12 + 기타.** Ⓑ의 근본 원인은 **순서**다 — `cleanupTopology`(연장·스냅)가 분할보다 **먼저** 돌아서 분할로 생긴 조각 끝점은 위상 정리를 못 받는다. 고치려면 **분할 후 위상 정리를 한 번 더** 돌려야 하는데 기하를 움직이므로 별도 검증 필요. (이번엔 Ⓐ만 수정하기로 결정)
