...
- **기둥 60 = 60 완전 일치.** 벽은 우리 92(분할 후, v0.42.0) vs 순정 103 → **누락 아님**(총연장 399m 온전), **분할 입도 차이**.
...
- **E8 B1F Y7~Y8 구역 "벽 인식 안 됨" 의심 사례 — 조사 결과 버그 아님**
  - 사용자가 캔버스 캡쳐에서 상단(Y7~Y8) 구역이 다른 구역보다 벽이 휑해 보인다고 지적 → 원본 DXF 레이어 단위로 전수 대조.
  - **밴드별 원본 `CON_WALL` 개수 → 추출된 벽 개수 비율**: Y7~Y8 = 14→7(**2.0:1**, 이중선 벽의 정상 페어링 비율). 다른 밴드(Y3~Y4 68→32=2.1:1, Y5~Y6 59→21=2.8:1)보다 오히려 손실이 적음. `CON_COLUMN`도 15→15(100%).
  - **실제 원인**: 그 구역엔 `U_wall_masonry`(조적벽) 11개가 섞여 있는데, `classifyLayer`가 MASONRY 키워드를 **의도적으로 null 처리**(비구조 조적/칸막이벽은 MIDAS 부재 아님) → 구조벽 대비 조적벽 비중이 높아 화면상 휑해 보였을 뿐, 인식 실패 아님.
  - **부수 발견(별개 이슈, B1F와 무관)**: `dwg sample/tracing test_story layer 1.dwg`는 **B2F·B1F·1F·2F 4개 층 평면이 한 DXF에 X축으로 나란히(총 1.45km 폭) 배치**돼 있음(`#B2F_S_WALL`·`#B1F_S_WALL`·`#1F_S_WALL`·`#2F_S_WALL` 레이어가 각각 100m+ 간격으로 분리). **크롭 없이 추출하면 그리드는 한 층 범위(65m)에서만 잡히는데 벽/기둥 노드는 전체 시트(1.45km)에서 끌려와 4개 층이 한 모델로 뭉개짐** — 오늘까지의 "tracing: 기둥150·벽92·grid18" 회귀 기준값은 이 뭉개진 상태 기준(무크롭↔무크롭 비교라 회귀 검증 자체는 유효했음). B2F 영역만 크롭(X 26만~39만)해서 재추출하면 기둥62·벽100·nodeXrange가 그 층 범위로 정상화됨. `CropPanel`의 "큰 도면 크롭 배너"(300m 초과 시 표시)가 이미 이 파일에 뜨므로, 이 파일로 테스트할 땐 **반드시 크롭 후 사용**할 것. (코드 수정은 하지 않음 — 배너로 충분하다고 판단, 필요시 후속 논의)
