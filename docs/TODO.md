...
> 기준 버전: **v0.36.0** · 갱신: 2026-07-05 · 배포: Vercel (`stru-xure-ai.vercel.app`)
...
- **C1 실도면 일반성** 🟢 **1차 검증·수정 완료(v0.33.0~v0.34.0)** — 중국법인 제공 실도면 6종(PKPM/YJK 관례)으로 검증. 발견·수정 4건:
  ...
  - ⚠️ 남음: 보 절점 분할(기둥/교차점에서 끊기 — 크롭 후에도 보가 그리드선따라 체인 병합됨), 边缘构件 상세표 vs 평면 구분, 이 파일 그리드(0轴线=6m 마커뿐, 데이터 없음).
