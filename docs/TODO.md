...
- **E3 오프셋 처리 방침** — ✅ **확정(2026-07-20, Gen NX API 직접 조회): 순정은 벽을 기둥/그리드에 스냅한다. 오프셋을 보존하지 않는다.**
  ...
  - → **E4에서 벽 그리드 이상화 구현.** 그 과정에서 **오프셋의 진짜 원인이 벽이 아니라 기둥이었음**이 드러남(E5).

- **E4 벽 그리드 이상화(v0.42.0)** [`idealizeWallsToGrid`] — 그리드축에 나란하고 가까운 벽의 **축선 전체를 그 축선 위로 평행이동**.
  - ⚠️ **접합 절점만 기둥으로 당기면 안 된다** — 직선 벽에 킨크가 생긴다. 반드시 축선 전체를 옮긴다. 순정도 벽을 통째로 그리드에 올린다(E3).
  - **판정 기준**: 그리드선이 **벽 몸통 안**에 있는가(수직거리 ≤ 두께/2, 상한 300mm). 두께에 자동 비례 → 얇은 벽을 억지로 끌어오지 않음. 사선 벽(skew>3%) 제외.
  - **위치**: `cleanupTopology` **앞**. 끝점이 기둥에 스냅된 뒤 축선을 옮기면 그 스냅이 어긋난다. 옵션 `idealizeToGrid`(기본 on), 카운트 `wallsIdealized`.
  - **결과**(B1F): 이상화 15건. **자유단 40→30**, 공유절점 65→70, 벽-기둥 절점일치 13→15, 절점 147→**140**(순정 평면 139와 거의 동일), 벽 92→93. 기둥 60·그리드 18 불변.
  - **회귀 없음**: zg 완전 동일(기둥18·벽16·두께250·grid22·절점30) · tracing 기둥공유 23→24, 벽 161→162(소폭 개선).

- **E5 ⚠️ 오프셋의 진짜 원인 = 기둥 위치(미해결, 결정 필요)**
  - E4 후에도 B1F 오프셋이 15→13에 그쳤고 **남은 13건이 전부 정확히 150mm**. 파보니 **벽은 이미 그리드 위에 정확히 있었고(Y1=-35125, 오차 0), 기둥 쪽이 150mm 벗어나 있었다.** 내 가정이 거꾸로였다.
  - 우리 기둥은 **자기들끼리도 불일치**: 하단열에서 `X2~X5-Y1`은 y=-34975, `X6/X8-Y1`은 y=-35125. 기둥 그리드 스냅(`SNAP=150mm`)이 경계값에서 일부만 발동.
  - **순정 실측 대조**: 순정은 하단열 **기둥도 벽도 전부 y=-35125**, X값도 기둥↔벽 완전 일치. **순정은 기둥까지 그리드에 올린다.**
  - **우리 기둥 60개 vs 순정 기둥 60개 위치 대조**: 완전일치 15 · ≤160mm 18 · **>160mm 27(최대 775mm)**, **중앙값 150mm**.
  - 🔜 **결정 필요**: 기둥도 그리드로 이상화할 것인가. 순정은 그렇게 한다. 다만 도면상 실제 기둥이 그리드에서 벗어나 있는 것(외곽기둥이 외피에 맞춰짐)이 **사실**일 수 있어, 이상화는 해석 편의를 위한 **모델링 판단**이다. 기둥 위치는 gridRef·단면·분할 등 전반에 영향 → **회귀 위험이 벽보다 훨씬 큼.** 미착수.
  ...
  - 📌 **다음(선택)**: 미연결 오프셋 지점을 검토 탭에서 **가시화**(자유단 amber와 유사)해 사용자가 Gen NX에서 판단하도록. 기하는 안 건드림.
  - 🔬 **확정용 Gen NX 확인 절차**: 순정 tracing 모델에서 이격 지점(예: `X1-Y2`, `X10-Y3`)의 **벽 코너 절점 번호와 기둥 절점 번호가 같은지** 조회. 같으면 순정이 스냅하는 것이고 방침 재검토 필요.
