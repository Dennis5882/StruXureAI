...
> 기준 버전: **v0.39.0** · 갱신: 2026-07-14 · 배포: Vercel (`stru-xure-ai.vercel.app`)
...
- **D1 도면 라벨(평법 집중표주) 파싱** 🟢 **1차 완료(v0.39.0)** [`parseBeamLabel` in geometry.ts] — 도면 텍스트가 설계 명시값을 담고 있어 기하 측정보다 정확하고, **평면 기하로는 불가능한 춤(depth)**까지 준다.
  ...
  - ⚠️ **라벨이 드러낸 기하 추출 실상(중요)**: 이 평면의 실제 보는 **~27개**인데 기하는 **197개** 생성(7배 과다), 폭도 **440/500/1000/1200 vs 실제 150~250**(2배+ 과다). → **보 면쌍 매칭이 잘못된 짝을 잡고 있음**. 다음 과제 = 라벨을 정답 삼아 보 면쌍 매칭 교정(또는 라벨 주도 추출).
