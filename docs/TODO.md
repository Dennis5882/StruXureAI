...
- **기둥 60 = 60 완전 일치.** 벽은 우리 101(E8 `hasPartner` 버그 수정 후, v0.42.0) vs 순정 103 → 개수 거의 근접, 남는 차이는 **분할 입도 차이**(누락 아님).
...
- **E7 코드리뷰 3건 수정 후 B1F 재검증(v0.42.0)** — 실행취소 desync·기둥 dedup 순서·`classifyMemberEnds` 캐싱 수정 뒤, "영상에서 순정이 인식한 것과 같은 기둥·벽 수량이 나오는가"를 다시 확인.
  - 재추출 결과: **기둥 60(gridRef 60/60)·벽 92·그리드 18·절점 131·벽 총연장 399m** — 수정 전(E6 검증 시점)과 **완전히 동일**. 회귀 없음.
  - 순정(영상) 대비: **기둥 60=60 완전 일치**, 벽은 92 vs 103(개수만 다름, 총연장 동일) → E1 상단 요약과 일치.

- **E8 B1F Y7~Y8 구역 "벽 인식 안 됨" 의심 사례 — 1차 조사는 오판, 2차 조사에서 진짜 버그 확인·수정(v0.42.0)**
  - 사용자가 캔버스 캡쳐에서 상단(Y7~Y8) 구역이 다른 구역보다 벽이 휑해 보인다고 지적.
  - **1차 조사(오판)**: 반경 기반으로 원본 `CON_WALL` 개수 대 추출 벽 개수 비율만 봤을 때(Y7~Y8 14→7 ≈ 2.0:1, 다른 밴드와 비슷) "정상"이라 결론 — **틀렸음**. 사용자가 순정 3D와 우리 모델 내보내기 3D를 나란히 비교하며 "외곽 둘레 벽 개수가 다르다"고 재차 지적해 재조사.
  - **2차 조사(진짜 원인)**: Y8 라인(외곽 둘레) 전 구간을 좌표로 직접 대조 — 원본 DXF는 그 라인을 따라 **거의 끊김없이 `CON_WALL`이 존재**(안쪽 면이 기둥마다 9조각으로 끊기고, 바깥쪽 면은 64.65m짜리 통선 하나)하는데, 우리 모델엔 딱 **2조각만** 추출돼 있었음(37~46m 분량 소실).
  - **근본 원인**: `geometry.ts`의 "단일선 벽 회복" 로직에서 `hasPartner(idx)`가 **이미 `pairFaces`에 소진된(used) 면을 후보에서 제외하지 않음**. "긴 통선 하나 + 기둥마다 끊긴 짧은 면 여러 개" 구도에서 통선이 그중 하나와만 짝지어지면, 나머지 짧은 면들은 "이미 남의 짝이 된 통선"을 여전히 유효한 파트너로 오인해 단일선 복구도 안 되고 통째로 버려짐.
  - **수정**: `hasPartner`의 후보 순회에 `wp.used.has(j)` 제외 조건 추가(1줄).
  - **효과**: B1F 벽 92→**101**(Y8 라인 W11~W20으로 완전 연결, 두께 322mm 일관), 기둥-벽 절점공유 28→**33**(의심했던 독립기둥 5개 C21·C24·C44·C48·C51 전부 정상 연결), 개방단부 12→10. tracing도 벽 증가(같은 패턴이 다른 파일에도 있었음). **zg는 완전 무변화**(회귀 없음 — 이 파일엔 해당 패턴이 없었을 뿐).
  - **교훈**: 반경/비율 기반 통계로 "정상"이라 판단한 1차 결론이 틀렸다 — **사용자가 시각적으로 지적한 특정 구간을 좌표로 직접 재구성**해서야 드러났다. 통계적 정상성 체크와 특정 구간 전수 대조는 서로 다른 걸 잡아낸다.
  - **부수 발견(별개 이슈, B1F와 무관)**: `dwg sample/tracing test_story layer 1.dwg`는 **B2F·B1F·1F·2F 4개 층 평면이 한 DXF에 X축으로 나란히(총 1.45km 폭) 배치**돼 있음(`#B2F_S_WALL`·`#B1F_S_WALL`·`#1F_S_WALL`·`#2F_S_WALL` 레이어가 각각 100m+ 간격으로 분리). 크롭 없이 추출하면 그리드는 한 층 범위(65m)에서만 잡히는데 벽/기둥 노드는 전체 시트(1.45km)에서 끌려와 4개 층이 한 모델로 뭉개짐. `CropPanel`의 "큰 도면 크롭 배너"(300m 초과 시 표시)가 이미 이 파일에 뜨므로, 이 파일로 테스트할 땐 **반드시 크롭 후 사용**할 것. (코드 수정은 하지 않음 — 배너로 충분하다고 판단, 필요시 후속 논의)

- **E9 배경 CAD에 그리드 버블 라벨·치수 문자 표시(v0.42.0)** [`Workspace.tsx`]
  - 사용자가 순정 원본 도면(그리드 버블에 X1·X2… 라벨, 그 사이 치수 숫자)과 우리 앱 화면(빈 원+숫자 없음)을 비교하며 "기본은 보여줘야 하지 않나" 지적.
  - **원인**: 배경 CAD 렌더 루프가 `TEXT`/`MTEXT`/`DIMENSION`을 성능·노이즈 방지 목적으로 통째로 스킵하고 있었음. `DIMENSION`은 로드 시 이미 `expandEntities`가 실제 `LINE`/`TEXT`로 전개해두므로, `TEXT`/`MTEXT`만 다시 그리면 치수 숫자까지 자동으로 함께 살아남.
  - **구현**: 배경 드로우리스트에 `dxfTextRef`(위치·회전·높이·색상)를 추가하고 `renderBg`에서 `ctx.fillText`로 그림. 화면상 높이 4px 미만이면(줌아웃) 스킵해 대형 도면(6.8만 엔티티) 성능·가독성 확보.
  - **검증**: B1F에서 그리드 라벨(X3~X10)·치수(8,200/7,025/…)가 원본과 동일하게 보임. tracing 파일(6.8만 엔티티) 줌아웃 상태에서 텍스트 자동 생략, 로드/렌더 지연 없음. 회귀 없음(B1F/zg 부재·절점 수 불변 — 표시 전용 변경이라 추출 로직 무관).

- **E10 `/code-review` 6건 중 실결함 2건 수정(v0.42.0)** — E9(텍스트 렌더) 직후 코드리뷰에서 나온 지적을 진짜 버그와 의도된 설계/저우선 사항으로 선별.
  - **고침 ①** [`fileLoader.ts` `transformEntity`]: 블록(`INSERT`) 내부 `TEXT`/`MTEXT`의 `rotation`을 블록 회전 변환에서 빠뜨리고 있었음 — 회전된 블록 안에 들어있는 라벨(예: 회전된 그리드 버블)이 원래 각도가 아니라 블록-로컬 각도 그대로 그려질 수 있는 결함. `out.rotation = e.rotation + rot*180/Math.PI` 추가.
  - **고침 ②** [`geometry.ts` `cleanText`]: MTEXT 문단 구분자 `\P`(세미콜론 없어서 기존 서식코드 제거 정규식에 안 걸림)를 줄바꿈으로 미변환 → 여러 줄 메모가 한 줄에 겹쳐 그려짐. `\P`→`\n` 치환 추가 + `Workspace.tsx` 렌더 루프에서 줄마다 `heightPx*1.2`씩 내려 그리도록 수정.
  - **보류(버그 아님/저우선)**: "그리드·치수 레이어만이 아니라 모든 TEXT/MTEXT를 그린다"는 지적은 E9에서 사용자가 직접 선택한 "원본 DXF TEXT/DIMENSION을 그대로 렌더" 방침 그 자체 — 결함 아님. `LAY_FOR_NGEN_*` 하드코딩 제외(일반화 대신 핀포인트 수정), MTEXT 방향벡터(그룹코드11) 회전 미지원, TEXT 정렬(halign/valign) 무시는 실사용 빈도 낮아 필요해지면 후속 처리.
  - **검증**: B1F 재추출 — 벽 101 / 기둥 60 / 노드 132 (불변), 콘솔 에러 없음, `npm run build` 정상.
