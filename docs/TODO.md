...
> 기준 버전: **v0.35.0** · 갱신: 2026-07-05 · 배포: Vercel (`stru-xure-ai.vercel.app`)
...
- **C1 실도면 일반성** 🟢 **1차 검증·수정 완료(v0.33.0~v0.34.0)** — 중국법인 제공 실도면 6종(PKPM/YJK 관례)으로 검증. 발견·수정 4건:
  ...
  - B1F/zg 전부 무회귀. ⚠️ **핵심 남음 = 크롭 리핏(Crop Refit)**: 거대 시공도 세트는 여러 평면이 흩어져 scale이 극소(0.0003) → 벽/보 이중선 면쌍매칭·그리드 축선이 전부 sub-pixel로 실패. 기둥만 자기완결 폴리라인이라 스케일 무관 정확. 현재 CROP은 엔티티만 필터하고 **transform 재계산 안 함** → 크롭해도 스케일 그대로. **크롭 영역으로 fit transform 재계산**하면 한 평면에서 그리드+보+정밀벽까지 살아남 = 실 시공도 활용의 관문. (그 위에 보 절점 분할=기둥/교차점에서 끊기도 필요.)
