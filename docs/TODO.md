...
- **E3 오프셋 처리 방침** — ⏸ **보류: 순정 Gen NX 동작 확인 대기.**
  - 벽 축선이 기둥 중심에서 **150mm** 벗어난 15건을 어떻게 다룰지(그리드 이상화 / 강체링크 / 미연결 유지)는 **구조 모델링 결정**이라 기하를 조용히 옮겨 해결하면 안 됨.
  - **먼저 확인할 것**: 순정 Tracing이 같은 이격 지점에서 벽 절점과 기둥 절점을 **공유하는가**. 영상 수치만으로는 판정 불가(§ COMPETITIVE_TRACING.md).
  - 다만 영상에서 역산되는 사실 하나: 순정 벽 고유 절점 160개 = 평면 **80점**으로 벽 103개를 구성(1.29벽/점). 우리는 평면 100점에 벽 92개(0.92). **순정 벽 네트워크가 우리보다 훨씬 조밀하게 연결돼 있다.**
